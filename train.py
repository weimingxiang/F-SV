...
if os.path.exists(data_dir + '/all_n_img' + '.pt') and not all_enforcement_refresh:
    ...
    for chromosome, chr_len in zip(chr_list, chr_length):
        ...
        if os.path.exists(data_dir + 'image/' + chromosome + '/positive_img' + '.pt') and not img_enforcement_refresh:
            ...
            negative_img_mid = torch.load(
                save_path + '/negative_img_mid' + '.pt')
        ...
        #     print("load end")
            ...
            for i, b_e in enumerate(ins_position):
                ...
                ins_img[i] = pool.apply_async(
                    ut.to_input_image_single, (chromosome_sign[:, b_e[0]:b_e[1]], )).get()
                # ins_img_mid[i] = ut.to_input_image_single(mid_sign[:, b_e[0]:b_e[1]]) # dim 4
                ins_img_mid[i] = pool.apply_async(
                    ut.to_input_image_single, (mid_sign[:, b_e[0]:b_e[1]], )).get()
                # ins_img_i[i] = resize(mid_sign_img[b_e[0]:b_e[1]].unsqueeze(0))
                ins_img_i[i] = pool.apply_async(
                    resize, (mid_sign_img[b_e[0]:b_e[1]].unsqueeze(0), )).get()
            ...
            torch.save(negative_img_mid, save_path +
                       '/negative_img_mid' + '.pt')
        ...
        all_negative_img_mid = torch.cat(
            (all_negative_img_mid, negative_img_mid), 0)
    ...
    all_ins_img = torch.cat(
        [all_ins_img, all_ins_img_mid, all_ins_cigar_img], 1)  # 3, 4, 3
    all_del_img = torch.cat(
        [all_del_img, all_del_img_mid, all_del_cigar_img], 1)  # 3, 4, 3
    all_n_img = torch.cat(
        [all_negative_img, all_negative_img_mid, all_negative_cigar_img], 1)
...
logger = TensorBoardLogger(os.path.join(
    "/home/xwm/DeepSVFilter/code", "channel_predict"), name=my_label)
...
def train_tune(config, checkpoint_dir=None, num_epochs=200, num_gpus=1):
    ...
    checkpoint_callback = ModelCheckpoint(
        dirpath="../../checkpoints_predict/" + my_label,
        filename=str(config["KFold_num"]) + "-" +
        '{epoch:02d}-{validation_f1:.2f}-{validation_mean:.2f}',
        monitor="validation_f1",
        verbose=False,
        save_last=None,
        save_top_k=1,
...
def gan_tune(num_samples=-1, num_epochs=30, gpus_per_trial=1):
    config = {
        ...
        "model_name": "resnet50",
        "KFold": 5,
        # "KFold_num":tune.randint(0, 5),# tune.randint(0, 4)
        "KFold_num": tune.choice([2]),
        # "KFold_num":tune.choice([1, 4]),
    }
    # config = {
    #     "batch_size": bs,
    #     "beta1": 0.9,
    ...
    reporter = CLIReporter(
        metric_columns=['train_loss', "train_f1",
