        ...
        self.layers = nn.ModuleList(
            nn.Sequential(
                nn.ConvTranspose2d(in_channels=m, out_channels=k,
                                   kernel_size=3, stride=1, padding=1),  # (m, 224, 224)
                nn.BatchNorm2d(k),
                nn.ReLU(inplace=True),
            ) for k, m in zip(dim1, dim2)
        )
        ...
        self.layers = nn.ModuleList(
            nn.Sequential(
                nn.Conv2d(in_channels=k, out_channels=k+1,
                          kernel_size=3, stride=1),  # (m, 224, 224)
                nn.BatchNorm2d(k+1),
...
class IDENet(pl.LightningModule):
    ...
    def __init__(self, path, config):
        ...
        self.resnet_model = eval(
            "torchvision.models." + self.model_name)(pretrained=True)  # [224, 224] -> 1000
        ...
        self.ms_ssim_module = MS_SSIM(
            data_range=255, size_average=True, channel=conv2d_dim[0])
        ...
        self.bert.embeddings.word_embeddings = nn.modules.sparse.Embedding(
            30000, 11, padding_idx=0)
        self.bert.embeddings.position_embeddings = nn.modules.sparse.Embedding(
            512, 11)
        self.bert.embeddings.token_type_embeddings = nn.modules.sparse.Embedding(
            2, 11)
        self.bert.embeddings.LayerNorm = nn.modules.normalization.LayerNorm(
            (11,), eps=1e-12, elementwise_affine=True)
        self.bert.encoder.embedding_hidden_mapping_in = nn.modules.linear.Linear(
            in_features=11, out_features=768, bias=True)
    ...
    def forward(self, x1, x2):
        ...
        reconstruct_loss = 1 - \
            self.ms_ssim_module(x1, re_x1) + self.L1(x1, re_x1) / 255
    ...
    def training_epoch_end(self, output):
        ...
        self.log('train_f1', metric['macro avg']['f1-score'],
    ...
    def validation_epoch_end(self, output):
        ...
        self.log('validation_f1', metric['macro avg']['f1-score'],
                 on_step=False, on_epoch=True, prog_bar=True, logger=True)
    ...
    def prepare_data(self):
        ...
        five_folds = StratifiedKFold(
            n_splits=self.KFold, shuffle=True, random_state=2022)
