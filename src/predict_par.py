...
# insert
filename = data_dir + "svim/variants.vcf"
# /home/xwm/DeepSVFilter/datasets/NA12878_PacBio_MtSinai/svim/variants.vcf
file = open(filename + "_filter.vcf", 'w')

count = 0
ins_img_i = torch.empty(64, 512, 11)
all_ins_img = torch.empty(64, 7, hight, hight)
all_data = [[] for _ in range(64)]
...
with open(filename, "r") as f:
    ...
    for data in lines:
        ...
        if "#" in data:
            ...
            ins_img_i[count] = resize(mid_sign_img[begin:end].unsqueeze(0))
            all_ins_img[count] = torch.cat([ins_img, ins_img_mid], 0)
            all_data[count] = data
            count += 1
            if(count == 64):
                count = 0
                y_hat = model(all_ins_img.cuda(), ins_img_i.cuda())
