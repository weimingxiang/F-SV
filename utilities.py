...
def load_class(path):
    print("Load dataset")
    insfile_list = os.listdir(path + "ins")
    delfile_list = os.listdir(path + "del")
    nfile_list = os.listdir(path + "n")

    _len = len(insfile_list) + len(delfile_list) + len(nfile_list)


    x_ids = range(_len)
    labels_items = [0] * len(insfile_list) + [1] * len(delfile_list) + [2] * len(nfile_list)

    return np.array(x_ids), np.array(labels_items)
